...
//  COMPLETED (10) Add constant values to sync Sunshine every 3 - 4 hours
    ...
    private static final int SYNC_INTERVAL_SECONDS = (int) TimeUnit.HOURS.toSeconds(SYNC_INTERVAL_HOUR);
    private static final int SYNC_FLEX_INTERVAL = SYNC_INTERVAL_SECONDS / 3;

    private static boolean sInitialized;

//  COMPLETED (11) Add a sync tag to identify our sync job
    private static final String WEATHER_JOB_TAG = "weather-job-tag";

//  COMPLETED (12) Create a method to schedule our periodic weather sync
    private static void scheduleWeatherJob (Context context) {
        Driver driver = new GooglePlayDriver(context);
        FirebaseJobDispatcher jobDispatcher = new FirebaseJobDispatcher(driver);
        Job weatherJob = jobDispatcher.newJobBuilder()
                .setService(SunshineFirebaseJobService.class)
                .setTag(WEATHER_JOB_TAG)
                .setConstraints(Constraint.ON_ANY_NETWORK)
                .setLifetime(Lifetime.FOREVER)
                .setRecurring(true)
                .setTrigger(Trigger.executionWindow(SYNC_INTERVAL_SECONDS,
                        SYNC_INTERVAL_SECONDS + SYNC_FLEX_INTERVAL))
